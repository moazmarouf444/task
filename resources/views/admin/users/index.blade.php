...
@section('content')
    ...
    <div class="layout-px-spacing">
        ...
        <div class="seperator-header">
            <a  data-route="{{route('admin.users.destroy_selected')}}" class="btn btn-danger mb-2 delete_all_button">
                حذف المحدد
                    ...
                    <div class="widget-content widget-content-area table-responsive">
                        <table id="style-3" class="table style-3  table-hover table-bordered table-striped table-checkable table-highlight-head mb-4">
                            ...
                            <tr>
                                ...
                                <th class="text-center">حاله الحظر</th>
                                <th class="text-center dt-no-sorting">التحكم</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach ($users as $user)
                                <tr class="delete_row checkbox-column" id="row-{{$user->id}}">
                                    <td class="text-center " data-id="{{ $user->id }}">
                                        @if ($user->id != 1 && auth()->id() != $user->id)
                                            <label class="new-control new-checkbox checkbox-primary" style="height: 18px; margin: 0 auto;">
                                                <input type="checkbox" class="new-control-input todochkbox checkSingle"  id="{{ $user->id }}">
                                    ...
                                    <td class="text-center">
                                            <span><img src="{{$user->avatar}}" class="profile-img"
                                                       alt="avatar"></span>
                                    </td>
                                    <td>{{$user->name}}</td>
                                    <td>{{$user->phone}}</td>
                                    ...
                                    <td class="text-center"><span
                                        @if ($user->is_blocked == 1)
                                            <span class="shadow-none badge badge-danger">محظور</span>
                                        @else
                                            <span class="shadow-none badge badge-primary">نشط</span>
                                        @endif
                                    </td>
                                    <td class="text-center product-action">
                                        <ul class="table-controls">
                                            ...
                                            @if ($user->id != 1 && auth()->id() != $user->id)
                                                <li>
                                                        <span class="delete-row bs-tooltip" data-url="{{ url('admin/user/' . $user->id) }}"
                                                              data-id="{{ $user->id }}">
